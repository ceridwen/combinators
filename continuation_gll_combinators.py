...
class Combinator(six.with_metaclass(abc.ABCMeta, object)):
    ...
    def parse(self, stream, index=0, greedy=False):
        # self.stack = []
        ...
        while gss.dispatch_stack:
            ...
            combinator._parse(gss, nonterminal_success, nonterminal_failure, stream, index)
        ...
        if successes:
            if greedy and index < len(stream):
                return Failure('Unexpected trailing characters: %r', stream, index)
            else:
    ...
    @abc.abstractmethod
    def _parse(self, gss, success, failure, stream, index):
        ...
        def continuation_factory(slot: int):
            if slot == len(self.combinators):
                def sequence_end(tree, failure, index):
                    trees.append(tree)
                    result = Success(tuple(trees), stream, index)
                ...
                def sequence_continuation(tree, failure, index):
                    trees.append(tree)
                    self.combinators[slot]._parse(gss, continuation_factory(slot + 1), failure, stream, index)
                return sequence_continuation
        continuation = continuation_factory(1)
        gss.create(self, continuation, failure, index)
        self.combinators[0]._parse(gss, continuation, failure, stream, index)
...
class Lazy(Combinator):
    ...
    def _parse(self, gss, success, failure, stream, index):
        ...
        combinator._parse(gss, success, failure, stream, index)
...
class Action(Combinator):
    ...
    def _parse(self, gss, success, failure, stream, index):
        ...
        self.combinator._parse(gss, action_continuation, failure, stream, index)
...
class Terminal(Sequence):
    ...
    def parse(self, stream, index=0, greedy=False):
        ...
        self._parse(GraphStructuredStack(self, index), terminal_success, terminal_failure, stream, index)
        return result
